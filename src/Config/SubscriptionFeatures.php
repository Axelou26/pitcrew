...
class SubscriptionFeatures
{
    ...
    public static function getAvailableFeatures(string $subscriptionLevel): array
    {
        ...
        if (!in_array($subscriptionLevel, self::SUBSCRIPTION_LEVELS)) {
            return [];
        }

        $availableFeatures = [];

        if (in_array($subscriptionLevel, [self::LEVEL_BASIC, self::LEVEL_PREMIUM, self::LEVEL_BUSINESS])) {
            $availableFeatures = array_merge($availableFeatures, self::FEATURES_BY_LEVEL[self::LEVEL_BASIC]);
        }

        if (in_array($subscriptionLevel, [self::LEVEL_PREMIUM, self::LEVEL_BUSINESS])) {
    ...
    {
        return in_array(strtolower($level), self::SUBSCRIPTION_LEVELS);
    }
    ...
    public static function isFeatureAvailableForLevel(string $feature, string $level): bool
    {
        ...
        return in_array(strtolower($feature), array_map('strtolower', $availableFeatures));
    }
}
